...
class ipoll64(connection):
    ...
    def changed_parameters(self, l):
        l['services']["ntp"]["servers"][0] = str("1.lt.pool.ntp.org")
      ...
      #  l['wireless']['radio'][0]['vap'][0]['ssid2vlan']['id'] = int(self.real_port)
        ...
        l['wireless']['radio'][0]['vap'][0]['mode'] = str("sta")
        l['wireless']['radio'][0]['vap'][0]['rate']['legacy'] = str('auto')
        l['wireless']['radio'][0]['vap'][0]['rate']['mcs'] = str('auto')
        l['wireless']['radio'][0]['wjet']['enabled'] = bool(True)
        l['wireless']['radio'][0]['wjet']["version"] = str("auto")
        l['wireless']['radio'][0]['autorate'] = str("rssi")
        l['wireless']['radio'][0]['ieeemode'] = str("an")



        #l['wireless']['radio'][0]['vap'][0]['security']['mode'] = str('wpapsk')
        #l['wireless']['radio'][0]['vap'][0]['security']['mode'] = str('wpaenterprise')
        ...
        l['wireless']['radio'][0]['vap'][0]['security']["wpaenterprise"]["accounting"]["servers"][0]["secret"] = str("testing123")
        l['wireless']['radio'][0]['vap'][0]['security']["wpaenterprise"]["accounting"]["servers"][0]["address"] = str("172.22.22.66")
        l['wireless']['radio'][0]['vap'][0]['security']["wpaenterprise"]["authentication"]["servers"][0]["secret"] = str("testing123")
        l['wireless']['radio'][0]['vap'][0]['security']["wpaenterprise"]["authentication"]["servers"][0]["address"] = str("172.22.22.66")
    
        #### WPA/PSK2
        l['wireless']['radio'][0]['vap'][0]['security']['wpapsk'] = {}
        l['wireless']['radio'][0]['vap'][0]['security']['wpapsk']['passphrase'] = 'testing123'
...
#for i in range(3, 7, 2):
    ...
    #threading.Thread(target = l.executing, args=['iwconfig ath0']).start()
    ...
    #threading.Thread(target = l.executing, args=['ip r; cat /usr/lib/version']).start()
    ...
    # Upgradina
    #threading.Thread(target = l.upgrade ,args=['/tmp/APCPE.QM-1.v7.55.39634.img']).start()
    #v = 35
    #if cc > v and cc < v+6:
    #    print(cc)
